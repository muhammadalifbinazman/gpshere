...
const createKnowledge = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO chatbot_knowledge (category, keywords, response, suggestions, priority, is_active) VALUES (?, ?, ?, ?, ?, ?)",
      [
        category,
        keywords || '',
        response,
...
const updateKnowledge = async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE chatbot_knowledge SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
      values
    );
